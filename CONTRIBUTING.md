# Contributing to Milpa Command

Thanks for your interest in contributing! Milpa Command is the Command-as-atom core of the
Milpa framework — one surface-agnostic `Operation` value object (schema of inputs + handler +
metadata) plus the `CommandProvider` and `SurfaceProjector` contracts everything else binds to.
It has **zero package dependencies** beyond PHP itself.
...
- **Respect the tier boundary.** Milpa Command is a zero-dependency leaf primitive —
  the `Operation` atom plus the `CommandProvider`/`SurfaceProjector` contracts; concrete
  surface projectors slot in behind it, host-side. It depends on nothing but PHP itself:
  no `milpa/core`, no kernel, no concrete projector. A change that would require adding
  a package dependency belongs in a different package, not here.
...
`milpa/data`, `milpa/command`) shares one coding standard, committed verbatim in every repo as
